...
Generator<string> ClassGrammarGenerator::get_next_fragment(string type) {
    if(type == "intf_method_modifier") {
        ...
        for(auto x : intf_modifiers)
            co_yield x;
    } else if(type == "intf") {
        for(auto x : match_string(valid_intf_template))
            co_yield x;
    } else if(type == "class_method_modifier") {
        for(auto x : class_method_modifiers)
            co_yield x;
    } else if(type == "class_member_modifier") {
        for(auto x : class_member_modifiers)
            co_yield x;
    } else if(type == "class_modifier") {
        for(auto x : class_modifiers)
            co_yield x;
    } else if(type == "class") {
        for(auto x : match_string(valid_class_template))
            co_yield x;
    }
}

...
ClassDecl::ClassDecl(Modifiers modifiers,
                     std::string name,
...
InterfaceDecl::InterfaceDecl(Modifiers modifiers,
                             std::string name,
...
MethodDecl::MethodDecl(Modifiers modifiers,
   ...
   if(modifiers.isAbstract() &&
      ...
      throw std::runtime_error(
            "An abstract method cannot be static, final or native. " + name);
   }
   ...
   if(modifiers.isNative()) {
      ...
      if(parameters.size() != 1) {
         throw std::runtime_error(
               "A native method must have exactly one parameter. " + name);
      }
      if(auto ty = dynamic_cast<BuiltInType*>(parameters[0]->getType())) {
         if(ty->getKind() != BuiltInType::Kind::Int) {
            throw std::runtime_error(
                  "A native method must have paramter type int. " + name);
         }
      }
   }
